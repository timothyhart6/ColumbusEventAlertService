...
public class KembaLiveEventServiceImplTest {
    @Mock
    ...
    @InjectMocks
    private KembaLiveEventServiceImpl subject;

    @BeforeEach
    public void setUp() {
        subject = new KembaLiveEventServiceImpl("Test", jsoupService, dateUtil, "Kemba");
    }
    ...
    public void testGetUpcomingEvent_success() throws Exception {
        ...
        when(jsoupService.getDocument(connection.userAgent(anyString())).getElementsByClass("doors-time").get(0).childNode(0).toString()).thenReturn("7:00 PM");
